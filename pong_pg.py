...
def OurModel(input_shape, action_space, lr):
    ...
    X = Dense(512, input_shape=input_shape, activation="elu", kernel_initializer='he_uniform')(X)
    ...
    model = Model(inputs = X_input, outputs = action, name='PNG_PG')
...
class agent_PG():
    ...
    def load(self, name):
        self.Actor = load_model(name, compile=False)
